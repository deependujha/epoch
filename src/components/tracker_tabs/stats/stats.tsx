...
import { ChartPieDonutText, PieDatum } from "@/components/charts/pie-chart";
...
const getDistinctDaysCount = (expenses: Expense[]) => {
    const days = new Set(
        expenses.map((e) => new Date(e.createdAt).toDateString()),
    );
    return days.size;
};
...
export const StatsTab = () => {
    ...
    useEffect(() => {
        ...
        fetch(
            `/api/expenses/month?year=${selectedMonth.year}&month=${selectedMonth.month}`,
        )
            .then((res) => res.json())
